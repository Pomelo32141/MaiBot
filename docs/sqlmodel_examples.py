...
def example_query_chat_streams():
    ...
    with get_session() as session:
        ...
        # 获取单条记录
        statement = select(ChatStreams).where(ChatStreams.stream_id == "example_stream_001")
...
def example_complex_query():
    ...
    with get_session() as session:
        # 连接查询 (如果有外键关系)
        # 这里展示基本的过滤和排序
        statement = select(Messages).where(Messages.is_command.is_(False)).order_by(Messages.time.desc()).limit(10)
...
def example_count_and_aggregation():
    ...
    with get_session() as session:
        ...
        # 按平台分组计数
        statement = select(ChatStreams.platform, func.count(ChatStreams.id).label("count")).group_by(
...
def example_pagination():
    ...
    with get_session() as session:
        ...
        statement = select(Messages).order_by(Messages.time.desc()).offset((page - 1) * page_size).limit(page_size)
