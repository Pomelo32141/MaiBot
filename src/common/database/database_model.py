...
def _remove_extra_columns(table_name: str, extra_fields: set[str]) -> None:
    ...
    for field_name in extra_fields:
        logger.warning(f"表 '{table_name}' 存在多余字段 '{field_name}'。SQLite 不支持删除列,请手动处理或重建表。")
...
def sync_field_constraints() -> None:
    ...
    try:
        ...
        for model in models:
            ...
            # 获取当前表结构
            current_schema = {col["name"]: col for col in inspector.get_columns(table_name)}
            ...
            for field_name, field_info in model.__fields__.items():
                ...
                if model_allows_null and not current_nullable:
                    constraints_to_fix.append(
                        {"field_name": field_name, "action": "allow_null", "current": "NOT NULL", "target": "NULL"}
                    )
                elif not model_allows_null and current_nullable:
                    constraints_to_fix.append(
                        {"field_name": field_name, "action": "disallow_null", "current": "NULL", "target": "NOT NULL"}
                    )
...
def _fix_table_constraints(table_name: str, model: type[SQLModel], constraints_to_fix: list[dict]) -> None:
    """
    修复表的字段约束。
    ...
    try:
        ...
        for constraint in constraints_to_fix:
            logger.info(f"已修复字段 '{constraint['field_name']}': {constraint['current']} -> {constraint['target']}")
...
def fix_image_id() -> None:
    ...
    try:
        with get_session() as session:
            # 查询所有 image_id 为空的记录
            images = session.query(Images).filter((Images.image_id == "") | (Images.image_id.is_(None))).all()
